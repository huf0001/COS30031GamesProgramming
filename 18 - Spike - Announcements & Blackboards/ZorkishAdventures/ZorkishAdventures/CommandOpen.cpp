...
std::string CommandOpen::Process(std::vector<std::string> input, World* world, Player* player)
{
	...
	if (!StringManager::Instance()->VectorContainsString(input, "with"))	//Player's input was "open containerName"
	{
		//container->SetIsOpen(true);
		Message* msg = new Message("OPEN", containerItem->GetID(), "container", (std::any*) new std::string("open"));
		Message* result = MessageManager::Instance()->SendMessage(msg);
		std::string output = std::any_cast<std::string>(*result->GetContent());
		...
		{
			return "Error: invalid message sent to / received from " + containerItem->GetID() + "'s component container.";
		}
	}
